...
for numbers in range(number_files):
    ...
    with open(budgetcsv, 'r') as csvFile:
        ...
        for i in csvReader:
            ...
            date.append(i[0])
            revenue.append(int(i[1]))
            TotalRev = TotalRev + int(i[1])

            # RevEnd = int(i[1])
            RevChg = (revenue[i] - revenue[i-1])
            # RevChg = RevEnd - RevBeg
            TotalRevChange += RevChg
    ...
    with open('financial_analysis_report_' + str(numbers+1) + '.txt', 'w') as text:
        ...
        text.write("    Greatest Increase in Revenue: " +
                   str(IncreaseDate) + " ($" + str(GIncrease) + ")\n")
        text.write("    Greatest Decrease in Revenue: " +
                   str(DecreaseDate) + " ($" + str(GDecrease) + ")\n\n")
